...
RouteModel::Node *RoutePlanner::NextNode() {
    std::sort(
        this->open_list.begin(), this->open_list.end(),
        // lambda compare expression
        [] (const auto *a, const auto *b) {
            float af_value = a->g_value + a->h_value;
            float bf_value = b->g_value + b->h_value;
            return af_value > bf_value;
        }
    );
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    while (current_node != nullptr) {
        ...
        // Check if start_node is reached
    ...
    // Reverse the vector, because the current_node (end_node) should be
